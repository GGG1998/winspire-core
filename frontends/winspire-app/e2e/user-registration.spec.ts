...
test.describe('User Registration', () => {
  ...
  test('should display user registration form', async ({ page }) => {
  ...
  test('should show error when passwords do not match', async ({ page }) => {
    ...
    await page.getByLabel('Nickname').fill(`user${timestamp}`);
  ...
  test('should successfully register a new user', async ({ page }) => {
    ...
    const testNickname = `user${timestamp}`;
    ...
    // For example, checking if logout button or user email appears
    await expect(page.getByRole('button', { name: /logout/i })).toBeVisible({ timeout: 5000 });
  });

  test('should successfully register a user with optional fields', async ({ page }) => {
    const timestamp = Date.now();
    const testEmail = `user${timestamp}@example.com`;
    const testPassword = 'TestPassword123!';
    const testNickname = `user${timestamp}`;
